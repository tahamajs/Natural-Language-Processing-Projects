...
\begin{itemize}
    \item Verify environment setup using the documented smoke dependencies.
    \item Run:
\begin{lstlisting}[language=bash]
python scripts/run_pipeline.py all-smoke
\end{lstlisting}
    \item Confirm generated artifacts:
\begin{itemize}
    \item \path{artifacts/q1_metrics.json}
    \item \path{artifacts/q2_metrics.json}
    \item \path{report/generated_metrics.tex}
    \item \path{report/figures/q1_em_comparison.png}
    \item \path{report/figures/q1_error_types.png}
    \item \path{report/figures/q2_ifeval_comparison.png}
\end{itemize}
    \item Confirm report build output:
\begin{itemize}
    \item \path{report/main.pdf}
\end{itemize}
    \item Optional deep reruns:
\begin{itemize}
    \item \texttt{python scripts/run\_pipeline.py full-q1 --compile-report}
    \item \texttt{python scripts/run\_pipeline.py full-q2 --compile-report}
\end{itemize}
\end{itemize}
